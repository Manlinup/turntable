...
class Turntable extends Controller{
    ...
    public function get_prize_list() {
        ...
        foreach ($data['list'] as $k => $v) {//过滤前端不应该显示的字段
...
//            return resultArray(['error' => '转盘活动已经结束']);
        ...
        $userStatus = $turntableModel->check_user_times($params['turntable_id'],$userId,$data['num_by_one'],$data['start_date'],$data['end_date'],$data['frequncy']);
    ...
    public function play_game() {
        ...
        if ($res == -1) {
            ...
            return resultArray(['error'=>'没有抽奖次数了']);
        } elseif ($res == -6) {
    ...
    public function save() {
        ...
        if (empty($params['shop_id']) || empty($params['title']) || empty($params['description']) || empty($params['num_by_one']) || empty($params['listorder']) || !isset($params['status']) || empty($params['start_date']) || empty($params['end_date']) || empty($params['frequncy'])) {
        ...
        $insertData = array(
            ...
            'created'       => time(),
            'frequncy'      => intval($params['frequncy'])
        );
